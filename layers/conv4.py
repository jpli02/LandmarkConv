...
__all__ = ["PConv2d4", "TopLeftPool", "TopRightPool", "BottomLeftPool", "BottomRightPool"]
...
class PConv2d4(nn.Conv2d):
    def __init__(self, *args, **kwargs):
        super(PConv2d4, self).__init__(*args, **kwargs)
        # according to the original paper, there are no gates.
    ...
    def forward(self, x):
        # pdb.set_trace()
        x = torch.cat([self.pools[i](x.chunk(4, 1)[i],guide=None) for i in range(4)], dim=1)
        x = super().forward(x)
        return x